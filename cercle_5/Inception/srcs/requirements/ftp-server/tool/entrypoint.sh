...
FTP_HOME=/var/www/html/ftp_uploads
...
if ! id "$FTP_USER" &>/dev/null 2>&1; then
	adduser -D -h "$FTP_HOME" -s /bin/false "$FTP_USER"
fi
...
find "$FTP_HOME" -type d -exec chmod 755 {} \;
find "$FTP_HOME" -type f -exec chmod 644 {} \;

# Créer le répertoire pour vsftpd
mkdir -p /var/run/vsftpd/empty

#on remplace le process shell par vsftpd, il devient le PID 1 du container
exec vsftpd /etc/vsftpd/vsftpd.conf -obackground=NO
